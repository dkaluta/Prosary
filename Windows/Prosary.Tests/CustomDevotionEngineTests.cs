...
public class CustomDevotionEngineTests : IClassFixture<PrayerPackLoaderFixture>
{
    ...
    public void TrisagionDefaultFormFollowsThePrayerLanguage()
    {
        ...
        Assert.Equal("קַדּישַת אַלָהָא", aramaic[0].Title);
        Assert.Equal(
        ...
        Assert.Equal("שוּבחָא לַאבָא", glory.Title);
        Assert.Equal(
    ...
    {
        var creed = BuildSteps("rosary", "arc").First(step => step.Title == "מהַימנִינַן");
        Assert.Equal(
    ...
    {
        var abun = BuildSteps("rosary", "arc").First(step => step.Title == "צלוּתָא מָרָנָיתָא");
        Assert.Equal(
    ...
    {
        var hailMary = BuildSteps("rosary", "arc").First(step => step.Title == "שלָם לֶך מַריַם");
        Assert.Equal(
    ...
    /// <summary>Both sourced Hebrew uses stay visible as separate prayer-language choices.</summary>
    [Fact]
    public void RitesAreListedUnderTheirLanguage()
    {
        ...
        Assert.Equal("עברית — נוסח השליחות", resolved.NativeName);
