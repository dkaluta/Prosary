 ...
 * star as already-favorited immediately, without waiting on the initial favorites fetch. Stations
 * of the Cross has no per-favorite language — it always follows the app default. */
...
fun StationsFlowScreen(prayer: Prayer? = null, onBack: () -> Unit) {
    ...
    LaunchedEffect(prayer) {
        ...
        steps = services.engine.buildSteps(Prayer(kind = PrayerKind.StationsOfTheCross, languageCode = LanguageCatalog.defaultSentinel))
        currentIndex = 0
        seasonColor = services.calendar.seasonColorToday()

        val all = runCatching { services.presetStore.all() }.getOrDefault(emptyList())
        matchingFavoriteId = all.firstOrNull { it.kind == PrayerKind.StationsOfTheCross }?.id
    }
    ...
    PrayerStepFlowScreen(
        ...
        topBarActions = {
            IconButton(onClick = {
                scope.launch {
                    matchingFavoriteId = toggleStationsFavorite(services, matchingFavoriteId)
                }
            }) {
                Icon(
...
private suspend fun toggleStationsFavorite(services: AppServices, currentFavoriteId: String?): String? {
