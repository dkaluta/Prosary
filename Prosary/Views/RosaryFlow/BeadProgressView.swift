...
#Preview("Narrow — 5 decades") {
    let steps = MockRosaryEngine().buildSteps(for: RosaryConfig(mysterySelectionMode: .specific, specificMysteryGroup: .joyful))
    let layout = BeadLayout.build(steps: steps, currentIndex: steps.count / 2, hasClosingCross: true)
    return BeadProgressView(layout: layout, isWide: false)
        .padding()
}

#Preview("Narrow — 20 decades (wraps)") {
    let steps = MockRosaryEngine().buildSteps(for: RosaryConfig(mysterySelectionMode: .twentyMystery))
...
#Preview("Wide — 5 decades, single minor column") {
    let steps = MockRosaryEngine().buildSteps(for: RosaryConfig(mysterySelectionMode: .specific, specificMysteryGroup: .sorrowful))
    let layout = BeadLayout.build(steps: steps, currentIndex: steps.count / 3, hasClosingCross: true)
    return BeadProgressView(layout: layout, isWide: true)
        .padding()
}
...
#Preview("Wide — 5 decades, split minor columns (short window)") {
    let steps = MockRosaryEngine().buildSteps(for: RosaryConfig(mysterySelectionMode: .specific, specificMysteryGroup: .sorrowful))
    let layout = BeadLayout.build(steps: steps, currentIndex: steps.count / 3, hasClosingCross: true)
    return BeadProgressView(layout: layout, isWide: true, hasRoomForSingleMinorColumn: false)
        .padding()
}
