...
Shared/tools/download-data.sh                                             # prime the scripture source cache for offline work
```

`download-data.sh` is twinned with `Download-Data.ps1` for Windows, as is
`make-prosaryprayer.sh` with `Make-ProsaryPrayer.ps1`.
