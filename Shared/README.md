...
| [`tools/`](tools/) | The tooling: `validate-devotion.py` (the format's enforcement), `make-prosaryprayer.sh` / `Make-ProsaryPrayer.ps1` (packers), `import-scripture.py` (Aramaic/Greek/Spanish scripture from checkable editions), `bump-version.py`, `reflow-prayers.py`, `audit-content.py`. Every Python tool is a self-contained [uv](https://docs.astral.sh/uv/) script — `uv run --script tools/<name>.py`. |
